...
"use strict";
    ...
    window.simpleidw = simpleHeat
}(),

Cartographer.HeatLayer = (Cartographer.Layer ? Cartographer.Layer : Cartographer.Class).extend({
    /*
    options: {
        opacity: 0.5,
        maxZoom: 18,
    ...
    */
    initialize: function (latlngs, options) {
        ...
        Cartographer.setOptions(this, options);
    },
    ...
    redraw: function () {
        if (this._idw && !this._frame && !this._map._animating) {
            this._frame = Cartographer.Util.requestAnimFrame(this._redraw, this);
        }
        return this;
    },
    ...
    onAdd: function (map) {
        ...
        if (map.options.zoomAnimation && Cartographer.Browser.any3d) {
    ...
    _initCanvas: function () {
        var canvas = this._canvas = Cartographer.DomUtil.create('canvas', 'leaflet-idwmap-layer leaflet-layer');

        var originProp = Cartographer.DomUtil.testProp(['transformOrigin', 'WebkitTransformOrigin', 'msTransformOrigin']);
        ...
        var animated = this._map.options.zoomAnimation && Cartographer.Browser.any3d;
        Cartographer.DomUtil.addClass(canvas, 'leaflet-zoom-' + (animated ? 'animated' : 'hide'));
    ...
    _reset: function () {
        ...
        Cartographer.DomUtil.setPosition(this._canvas, topLeft);
    ...
    _redraw: function () {
        ...
        var data = [],
            ...
            bounds = new Cartographer.Bounds(
                Cartographer.point([-r, -r]),
                size.add([r, r])),
        ...
        for (i = 0, len = nCellY; i < len; i++) {
            ...
            for (j = 0, len2 = nCellX; j < len2; j++) {     
                ...
                for (k = 0, len3 = this._latlngs.length; k < len3; k++) {          
                    ...
                    var cp = Cartographer.point((y-cellCen), (x-cellCen));                    
    ...
    _animateZoom: function (e) {
        ...
        if (Cartographer.DomUtil.setTransform) {
            Cartographer.DomUtil.setTransform(this._canvas, offset, scale);

        } else {
            this._canvas.style[Cartographer.DomUtil.TRANSFORM] = Cartographer.DomUtil.getTranslateString(offset) + ' scale(' + scale + ')';
        }
    }
});

Cartographer.heatLayer = function (latlngs, options) {
    return new Cartographer.HeatLayer(latlngs, options);
};